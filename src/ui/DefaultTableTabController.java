    ...
    model.addColumnListener(new SetChangeListener<Column>() {
      ...
      public void onChanged(SetChangeListener.Change<? extends Column> change) {
        if (change.wasAdded()) {
          ...
          //defaultValueの整備
          StringProperty columnNameProp = new SimpleStringProperty("newValue");
          defaultValue.getValueMap().put(columnName, columnNameProp);

          column.nameProperty().addListener((observable, oldValue, newValue) -> {
            StringProperty value = defaultValue.getValueMap().get(oldValue);
            defaultValue.getValueMap().put(newValue, value);
            defaultValue.getValueMap().remove(oldValue);
          });
          ...
          addColumn.idProperty().bind(column.nameProperty());
          addColumn.setCellFactory(TextFieldTableCell.<DefaultRow> forTableColumn());
          addColumn.setCellValueFactory((CellDataFeatures<DefaultRow, String> p) -> {
            return p.getValue().getValueMap().get(column.getName());
          });
          ...
          addColumn.setOnEditCommit(event -> {
            ...
            //TODO modelTable側に設定したリスナーからの通知で値を変更するようにしたい
              ...
              event.getRowValue().getValueMap().get(column.getName()).set(event.getNewValue());
            });
